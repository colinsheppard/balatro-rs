...
impl TomlJokerParser {
    ...
    pub fn parse_directory<P: AsRef<Path>>(&self, dir_path: P) -> Result<JokerConfig, TomlParserError> {
    ...
    fn joker_definition_changed(&self, old: &TomlJokerDefinition, new: &TomlJokerDefinition) -> bool {
    ...
    fn validate(&self, config: &JokerConfig) -> Result<(), TomlParserError> {
        for joker in &config.jokers {
            ...
            if joker.description.is_empty() {
                return Err(TomlParserError::MissingRequiredField("description".to_string()));
            }
...
impl JokerValidator for RangeValidator {
    fn validate(&self, config: &JokerConfig) -> Result<(), TomlParserError> {
        for joker in &config.jokers {
            ...
            match &joker.effect {
                crate::joker_toml_schema::TomlJokerEffect::Scoring { chips, mult, money, mult_multiplier, .. } => {
...
impl JokerValidator for LogicValidator {
    fn validate(&self, config: &JokerConfig) -> Result<(), TomlParserError> {
        for joker in &config.jokers {
            ...
            if let crate::joker_toml_schema::TomlJokerEffect::Conditional { condition, .. } = &joker.effect {
...
impl LogicValidator {
    fn validate_condition(&self, _condition: &crate::joker_toml_schema::TomlJokerCondition) -> Result<(), TomlParserError> {
...
mod tests {
    ...
    fn test_hot_reload_detection() {
        ...
        let (_, changed) = parser.parse_with_hot_reload(modified, Some(&original_config)).unwrap();
