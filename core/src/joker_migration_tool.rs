...
impl JokerMigrationTool {
    ...
    pub fn add_custom_migrator(&mut self, joker_id: JokerId, migrator: Box<dyn CustomJokerMigrator>) {
    ...
    fn infer_effect_type(&self, joker: &dyn Joker) -> Option<TomlJokerEffect> {
        ...
        match joker_id {
            ...
            JokerId::GreedyJoker => Some(TomlJokerEffect::Conditional {
                condition: TomlJokerCondition::SuitScored { suit: TomlSuit::Diamonds },
            ...
            JokerId::LustyJoker => Some(TomlJokerEffect::Conditional {
                condition: TomlJokerCondition::SuitScored { suit: TomlSuit::Hearts },
                action: TomlJokerAction::AddScore {
                    chips: 0,
                    mult: 3,
                    money: 0,
            ...
            JokerId::WrathfulJoker => Some(TomlJokerEffect::Conditional {
                condition: TomlJokerCondition::SuitScored { suit: TomlSuit::Spades },
                action: TomlJokerAction::AddScore {
                    chips: 0,
                    mult: 3,
                    money: 0,
            ...
            JokerId::GluttonousJoker => Some(TomlJokerEffect::Conditional {
                condition: TomlJokerCondition::SuitScored { suit: TomlSuit::Clubs },
                action: TomlJokerAction::AddScore {
                    chips: 0,
                    mult: 3,
                    money: 0,
            ...
            JokerId::JollyJoker => Some(TomlJokerEffect::Conditional {
                condition: TomlJokerCondition::HandType { hand_type: TomlHandRank::Pair },
                action: TomlJokerAction::AddScore {
                    chips: 0,
                    mult: 8,
                    money: 0,
    ...
    pub fn save_to_file<P: AsRef<std::path::Path>>(&self, config: &JokerConfig, path: P) -> Result<(), Box<dyn std::error::Error>> {
    ...
    fn migrate(&self, joker: &dyn Joker) -> Option<TomlJokerDefinition> {
        ...
        Some(TomlJokerDefinition {
            ...
            effect: TomlJokerEffect::Dynamic {
                ...
                state_modifiers: vec![
                    crate::joker_toml_schema::TomlStateModifier {
            ...
            state: Some(crate::joker_toml_schema::TomlJokerState {
                fields: {
                    ...
                    fields.insert("hands_played".to_string(), crate::joker_toml_schema::TomlValue::Integer(0));
                    fields
                },
                persistent: true,
            }),
...
mod tests {
    use super::*;
    use crate::joker_impl::{TheJoker, GreedyJoker};
    ...
    fn test_migrate_conditional_joker() {
        ...
        if let TomlJokerEffect::Conditional { condition, action, per_card } = definition.effect {
            assert!(per_card);
